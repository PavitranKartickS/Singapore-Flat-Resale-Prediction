...
st.set_page_config(page_title="Singapore Flat Resale Prediction ",
                   layout="wide",
...
st.markdown("<h1 style='text-align: center; color: #e5c54f;'>GUVI Capstone Project 6</h1>",unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center; color: #e5c54f;'>Singapore Flat Resale Prediction</h1>",unsafe_allow_html=True)


selected = option_menu(
    menu_title=None,
    options=["Introduction", "Prediction", "Inference"],
    icons=["info-circle", "patch-question", "hourglass-split"],
    default_index=0,
    orientation="horizontal",
    styles={"container": {"padding": "0!important", "background-color": "#16537e", "size": "cover", "width": "100%"},
            "icon": {"color": "black", "font-size": "20px"},
            "nav-link": {"font-size": "20px", "text-align": "center", "margin": "-2px", "--hover-color": "#00aced"},
            "nav-link-selected": {"background-color": "#00aced"}})
...
if selected == "Introduction":
    ...
    with c1:
        st.write("### Singapore Housing Scenario:")
        st.write("- In Singapore, over 80 per cent of residents live in high-rise public housing blocks provided by the Housing and Development Board on leasehold land. Affordability and public housing availability is impacted by factors such as land availability, construction costs and supply chain disruptions, and population growth. But despite measures including government subsidies, loans and housing grants to offset rising prices, affordability has declined as evidenced by increased price-to-income ratios from 2001-2021.")
        st.write("- This project aims to provide a method to predict the resale prices of flats in singapore through a user friendly Web application. ")
...
if selected == "Prediction":
    ...
    def main():
        st.sidebar.title('Enter Details of desirable property for prediction')
        ...
        town_mapping = {'ANG MO KIO': 1, 'BEDOK': 2, 'BISHAN': 3, 'BUKIT BATOK': 4, 'BUKIT MERAH': 5, 'BUKIT TIMAH': 6,
                        ...
                        'JURONG EAST': 12, 'JURONG WEST': 13, 'KALLANG/WHAMPOA': 14, 'MARINE PARADE': 15,
                        'QUEENSTOWN': 16,
        ...
        streets = {'ANG MO KIO AVE 1': 1, 'ANG MO KIO AVE 3': 2, 'ANG MO KIO AVE 4': 3, 'ANG MO KIO AVE 10': 4,
                   ...
                   'BEO CRES': 42,
                   'TELOK BLANGAH DR': 43, 'DEPOT RD': 44, 'TELOK BLANGAH RISE': 45, 'JLN BT MERAH': 46,
                   'HENDERSON RD': 47,
                   'INDUS RD': 48, 'BT MERAH VIEW': 49, 'HENDERSON CRES': 50, 'BT PURMEI RD': 51,
                   ...
                   'JLN KUKOH': 69,
                   'OUTRAM PK': 70, 'SHORT ST': 71, 'SELEGIE RD': 72, 'UPP CROSS ST': 73, 'WATERLOO ST': 74,
                   'QUEEN ST': 75,
                   'BUFFALO RD': 76, 'ROWELL RD': 77, 'ROCHOR RD': 78, 'BAIN ST': 79, 'SMITH ST': 80,
                   'VEERASAMY RD': 81,
                   ...
                   'GEYLANG SERAI': 100,
                   'EUNOS CRES': 101, 'SIMS DR': 102, 'ALJUNIED CRES': 103, 'GEYLANG EAST AVE 1': 104,
                   'DAKOTA CRES': 105,
                   ...
                   'TEBAN GDNS RD': 121,
                   'JURONG EAST AVE 1': 122, 'JURONG EAST ST 32': 123, 'JURONG EAST ST 13': 124,
                   ...
                   "C'WEALTH DR": 166, 'TANGLIN HALT RD': 167, "C'WEALTH CRES": 168, 'DOVER RD': 169,
                   'MARGARET DR': 170,
                   ...
                   'YISHUN ST 72': 221,
                   'YISHUN ST 11': 222, 'YISHUN ST 21': 223, 'YISHUN ST 22': 224, 'YISHUN AVE 3': 225,
                   'CHAI CHEE AVE': 226,
                   ...
                   'TAO CHING RD': 237, 'GLOUCESTER RD': 238, 'BOON KENG RD': 239, 'WHAMPOA STH': 240,
                   'CAMBRIDGE RD': 241,
                   'TAMPINES ST 42': 242, 'LOR 6 TOA PAYOH': 243, 'KIM KEAT AVE': 244, 'YISHUN AVE 6': 245,
                   'YISHUN AVE 9': 246,
                   'YISHUN ST 71': 247, 'BT BATOK ST 32': 248, 'SILAT AVE': 249, 'TIONG BAHRU RD': 250,
                   'SAGO LANE': 251,
                   ...
                   'TAMPINES ST 43': 292, 'BISHAN ST 13': 293, 'JLN DUSUN': 294, 'YISHUN AVE 2': 295,
                   'JOO CHIAT RD': 296,
                   'EAST COAST RD': 297, 'REDHILL RD': 298, 'KIM PONG RD': 299, 'RACE COURSE RD': 300,
                   'KRETA AYER RD': 301,
                   ...
                   'GANGSA RD': 311, 'MCNAIR RD': 312, 'SIMEI ST 4': 313, 'YISHUN AVE 7': 314,
                   'SERANGOON NTH AVE 2': 315,
                   'YISHUN AVE 11': 316, 'BANGKIT RD': 317, 'JURONG WEST ST 73': 318, 'OUTRAM HILL': 319,
                   'HOUGANG AVE 6': 320,
                   'PASIR RIS ST 12': 321, 'PENDING RD': 322, 'PETIR RD': 323, 'LOR 3 GEYLANG': 324,
                   'BISHAN ST 11': 325,
                   ...
                   'SERANGOON AVE 1': 359, 'PASIR RIS ST 41': 360, 'GEYLANG EAST AVE 2': 361,
                   ...
                   'ELIAS RD': 388, 'BISHAN ST 24': 389, 'WHAMPOA RD': 390, 'WOODLANDS ST 41': 391,
                   'PASIR RIS ST 71': 392,
                   ...
                   'SENJA LINK': 499, 'EDGEFIELD PLAINS': 500, 'ANCHORVALE DR': 501, 'SEGAR RD': 502,
                   'FARRER PK RD': 503,
                   ...
                   'KLANG LANE': 513, 'PUNGGOL CTRL': 514, 'JELEBU RD': 515, 'BUANGKOK CRES': 516,
                   'WOODLANDS DR 75': 517,
                   'BT BATOK WEST AVE 5': 518, 'JELLICOE RD': 519, 'PUNGGOL DR': 520, 'JURONG WEST ST 24': 521,
                   'SEMBAWANG WAY': 522,
                   'FERNVALE RD': 523, 'BUANGKOK LINK': 524, 'FERNVALE LINK': 525, 'JLN TIGA': 526,
                   'YUAN CHING RD': 527,
        ...
        # get block details
        block_ = st.sidebar.text_input('Enter the block number (eg.201A)', value=254)
        # Define a mapping for letters to decimal values
        letter_mapping = {chr(ord('A') + i): f'.{i + 1}' for i in range(26)}
        block = float(''.join(letter_mapping.get(c, c) for c in block_))
        ...
        flat_type_value = st.sidebar.selectbox('Select Flat Type', list(category_mapping.keys()))
        ...
        flat_model_mapping = {'IMPROVED': 1, 'NEW GENERATION': 2, 'MODEL A': 3, 'STANDARD': 4, 'SIMPLIFIED': 5,
                              ...
                              'Maisonette':
                                  21, 'Multi Generation': 22, 'Adjoined flat': 23, 'Premium Apartment': 24,
                              'Terrace': 25,
                              'Improved-Maisonette': 26, 'Premium Maisonette': 27, '2-room': 28, 'Model A2': 29,
                              'DBSS': 30,
                              'Type S1': 31, 'Type S2': 32, 'Premium Apartment Loft': 33, '3Gen': 34}

        flat_model_value = st.sidebar.selectbox("Select Flat Model", list(flat_model_mapping.keys()))
        flat_model = flat_model_mapping[flat_model_value]
        # input for area in sq.m
        floor_area_sqm = st.sidebar.number_input("Enter the area", value=35.0)
        # storey range input
        storey_lower_bound = st.sidebar.number_input("Enter the lower bound of the storey range", value=4, min_value=0)
        storey_upper_bound = st.sidebar.number_input("Enter the upper bound of the storey range", value=6,
                                                     min_value=storey_lower_bound)
        # input for lease commence year
        lease_commence_date = st.sidebar.number_input("Enter the lease commence year", value=1990)



        resale_year = st.sidebar.number_input('Enter the resale year', value=1990, min_value=lease_commence_date)
...
if selected == "Inference":
    ...
    st.write("- As previously displayed, the resale price of Flats in Singaporean neighbourhoods can be predicted based on the provided data. It does not mention which of these factors affect the value of resale price. ")
    st.write("- We can study the impact of each of these features with the help of the following:")
    ...
    st.write("### Conclusion:")
    st.write("- The features affecting the Resale Price are: floor area sqm, flat type, town, lease commence date, street name, block, flat model, resale year, storey lower bound, resale month in that order of importance.")